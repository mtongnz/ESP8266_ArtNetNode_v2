...
/*
  More pixels need more ram! for longer chains make sure to use sdk > 2.4.1 with lwip2 low mem

  Http interface can still cause crashes, normal operation seems solid now.

  --->>>> Make sure to RESTART the esp once AFTER serial FLASHING, esp.restart fails if you dont.

  todo:
  Sometimes we get stuck in a reboot loop, seems like wifi state gets messed up.
  ws2812 needs 160mhz currently, calculate timing nops count in precompiler
  calculate heap usage and check if setting is possible.
  There seems to be another mem leak left
...
#define FIRMWARE_VERSION "v2.0.0d (beta 5g)"
...
/*
   ...
   //W works for strip on esp12, R works as pwm out
   SD2 (GPIO 10) //can be used on esp-12F in DIO mode, otherwise used for flash.
  D0 (GPIO 16) R
  D1 (GPIO 5) RW -- Default OUT A

  D2 (GPIO 4) RW -- Default OUT B
  D3 (GPIO 0) R  --problematic when reset, boot mode pin -- maybe not?!?
  D4 (GPIO 2) RW
  ...
  D8 (GPIO 15) R--problematic when reset, boot mode pin -- maybe not?!?
  RXD (GPIO 3) Somewhat strange, probably need to disconnect rs232 chip.
  TXD (GPIO 1) RW

*/
const int pwmports[] = {16, 2, 14, 12, 13};
//const int pwmports[] = {5, 16, 10, 4, 0, 2, 14, 12, 13, 15, 3, 1};//make all unused ports pwm outs
const int pwmcount = sizeof( pwmports ) / sizeof( int );
int lastvalues[pwmcount] = {};
#define RAW_UNIVERSE 10 //start on universe 10, we dont want to conflict with the strips below
espDMX RAWPort(2);

//#define DMX_OFF //disable dmx handler a/b, free up those pins
#ifndef ULONG_MAX
  #define ULONG_MAX  4294967295UL
#endif

#ifdef ESP_01
  #define DMX_DIR_A 255   // Tie high on max585, 255 disables gpio
  #define DMX_TX_A 1
  #define ONE_PORT
  #define NO_RESET
...
#else
  #define DMX_DIR_A 255   // Tie high on max585, 255 disables gpio
  #define DMX_DIR_B 255   // Tie high on max585, 255 disables gpio
  #define DMX_TX_A 5 //output A pin, not necessary dmx.
  #define DMX_TX_B 4 //output B pin, not necessary dmx.
  #define NO_RESET
...
uint8_t portA[5], portB[5], portC[5];
uint8_t MAC_array[6];
...
//get webserver data from spiffs (data folder), this allows gzip compressed content. smaller files=shorter connection and less heap usage.
//also disables upload of spiffsdata
#define spiffs
...
const char PROGMEM heapCriticalRestart[] = "memory critical, performing restart";
...
void setup(void) {
  ...
  void wifi_status_led_uninstall(); //make gpio 0 usable
  ...
  */
  ...
  // Check if SPIFFS formatted ... just why???
  /*
  ...
  */
  ...
  if (!deviceSettings.doFirmwareUpdate) {
    ...
    for (int i = 0; i < pwmcount; i++) {
      ...
      analogWrite(pwmports[i], PWMRANGE); //using common anode, high is off
    }
    portC[0] = artRDM.addGroup(deviceSettings.portAnet, deviceSettings.portAsub);
    portC[1] = artRDM.addPort(portC[0], 0, RAW_UNIVERSE, TYPE_DMX_OUT, deviceSettings.portAmerge); //group?,0,universe,type,mergemode)
    RAWPort.begin(255, artRDM.getDMX(portC[0], portC[1]));//(uint8_t dir, byte* buf)
    RAWPort.rdmEnable(ESTA_MAN, ESTA_DEV);
    RAWPort.rdmSetCallBack(rdmReceivedC);
    RAWPort.todSetCallBack(sendTodC);
  } else {
...
void rdmReceivedC(rdm_data* c) {
  artRDM.rdmResponse(c, portC[0], portC[1]);
}

void sendTodC() {
  artRDM.artTODData(portC[0], portC[1], RAWPort.todMan(), RAWPort.todDev(), RAWPort.todCount(), RAWPort.todStatus());
}
unsigned long lastheaptime;

unsigned long lowheap = ULONG_MAX  ;
unsigned long lastheaplow = ULONG_MAX;
void loop(void) {
  ESP.wdtFeed();
  yield();
  ...
  if (ESP.getFreeHeap() < lowheap) lowheap = ESP.getFreeHeap();
  if (millis() - lastheaptime > 1000 && lastheaplow != lowheap)
  {
    Serial.printf("millis: %u heap size: %u\n", millis(), lowheap);
    lastheaptime = millis();
    lastheaplow = lowheap;
  }
  if (lowheap < 24000) {
    tcpCleanup();
    lowheap = ESP.getFreeHeap();
  }
  if (lowheap < 10000) {
    Serial.println(heapCriticalRestart);
    WiFi.forceSleepBegin();
    WiFi.disconnect();
    ESP.restart();
    delay(1000);
...
void tcpCleanup(void)   // losing bytes work around
{
  while (tcp_tw_pcbs)
  {
