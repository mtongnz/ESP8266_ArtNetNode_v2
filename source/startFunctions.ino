...
const char PROGMEM  cacheline[] = "public, max-age=86400, must-revalidate";

const char PROGMEM statusOK[] = "OK: Device started";
const char PROGMEM errorHWDT[] = "ERROR: (HWDT) Unexpected device restart";
const char PROGMEM errorEXCP[] = "ERROR: (EXCP) Unexpected device restart";
const char PROGMEM errorSWDT[] = "ERROR: (SWDT) Unexpected device restart";
const char PROGMEM errorHWDTshort[] = "Restart error: HWDT";
const char PROGMEM errorEXCPshort[] = "Restart error: EXCP";
const char PROGMEM errorSWDTshort[] = "Error on Restart: SWDT";
...
void artStart() {
  ...
  switch (resetInfo.reason) {
    ...
    case REASON_SOFT_RESTART:
      artRDM.setNodeReport(statusOK, ARTNET_RC_POWER_OK);
      nextNodeReport = millis() + 4000;
      break;

    case REASON_WDT_RST:
      artRDM.setNodeReport(errorHWDT, ARTNET_RC_POWER_FAIL);
      strcpy(nodeError, errorHWDTshort );
      nextNodeReport = millis() + 10000;
      nodeErrorTimeout = millis() + 30000;
      break;
    case REASON_EXCEPTION_RST:
      artRDM.setNodeReport(errorEXCP, ARTNET_RC_POWER_FAIL);
      strcpy(nodeError, errorEXCPshort );
      nextNodeReport = millis() + 10000;
      nodeErrorTimeout = millis() + 30000;
      break;
    case REASON_SOFT_WDT_RST:
      artRDM.setNodeReport(errorSWDT, ARTNET_RC_POWER_FAIL);
      strcpy(nodeError, errorSWDTshort);
      nextNodeReport = millis() + 10000;
      nodeErrorTimeout = millis() + 30000;
      break;
...
void webStart() {
  ...
  //todo, compare performance of streamfile to serve static
  //serveStatic is nice!
  /*
  ...
  */
  ...
  webServer.serveStatic("/", SPIFFS, "/", cacheline);
